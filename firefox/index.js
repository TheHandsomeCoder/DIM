...
var item_data = JSON.parse(data.load('app/scripts/api-manifest/items.json'));
...
function getBungieCookies() {
    ...
    var cookieValue = "",
        cookieMgr = Cc["@mozilla.org/cookiemanager;1"].getService(Ci.nsICookieManager);
...
tabs.on('ready', function(tab) {
    ...
    worker = tab.attach({ //TODO: Find a more elegent way of passing the cookie to the extention. This only works if the cookie already exists.
        ...
        contentScriptFile: [
            ...
            data.url("./app/scripts/services/dimObjectiveDefinitions.factory.js"),
            data.url("./app/scripts/services/dimSandboxPerkDefinitions.factory.js"),
    ...
    worker.port.on("request-item-definitions", function() {
        console.log("index.js received item definitions request");
        worker.port.emit('item-data', item_data);
    });

    worker.port.on("request-objective-definitions", function() {
        console.log("index.js received objective definitions request");
        worker.port.emit('objective-data', objective_data);
    });

    worker.port.on("request-sandbox-definitions", function() {
        console.log("index.js received sandbox definitions request");
        worker.port.emit('sandbox-data', sandbox_data);
    });
